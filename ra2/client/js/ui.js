...
function showHit(zone) { const h = $('hitmark'); h.classList.toggle('hs', zone === 'head'); h.classList.remove('pop'); void h.offsetWidth; h.classList.add('pop'); zone === 'head' ? AUDIO.headshot() : zone === 'torso' ? AUDIO.hit() : AUDIO.hitLimb(); }
function showKill() { const k = $('killx'); k.classList.remove('pop'); void k.offsetWidth; k.classList.add('pop'); }
...
document.addEventListener('mousedown', e => {
  ...
  AUDIO.resume();
  if (e.button === 0 && (INPUT.locked || INPUT.fallback)) { INPUT.fireHeld = true; tryFire(); }
...
document.addEventListener('keydown', e => {
  ...
  if (e.code === 'KeyQ' || e.code === 'KeyF') { /* reserved: weapon inspect / melee quick-swap in a later pass */ }
