...
function buildMap() {
  ...
  // ---- M12: mid-lane crouch cover (0.9 m: crouch behind, mantle over), wooden crate chain up to the SE container stack
  for (const [x, z, rot] of [[-8, -16, 0], [8, 16, 0], [-19, -2, 1], [19, 2, 1], [0, -15, 0], [0, 15, 0]]) block(x, 0, z, rot ? 0.5 : 3.2, 0.9, rot ? 3.2 : 0.5, MAT.concrete);
  block(-23.3, 0, 12, 1.6, 0.9, 1.6, MAT.wood); block(-21.7, 0, 12, 1.4, 1.8, 1.6, MAT.wood); // 0.9 -> 1.8 -> single container top (2.6) at (-18,12): jump-mantle route, west wall x=-21
  block(22.5, 0, -12, 1.6, 0.9, 1.6, MAT.wood); block(20.9, 0, -12, 1.4, 1.8, 1.6, MAT.wood);  // mirror onto the single container at (19,-12), east wall x=20.2
  block(-6, 3.7, 4, 1.2, 0.9, 1.2, MAT.wood); block(6, 3.7, -4.5, 1.2, 0.9, 1.2, MAT.wood);   // deck-1 crouch cover
  // ---- spawn points: corners at ground, both towers, both catwalks, two mid perches
  SPAWNS.push(sp(-25, 0, 25), sp(25, 0, -25), sp(-25.5, 0, -12), sp(25.5, 0, 12),
